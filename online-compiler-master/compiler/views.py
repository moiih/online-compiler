...
###     Need To Be Refreshed On Login.

###     Check "get_code" URL on clicking on it, not working properly !?!
...
#         file_id = CodeFileUpload.objects.get(pk=id)
#         print('\n\nFile_Id (get code) :', file_id.codefile.url, end='\n\n')
#         file_nm = file_id.codefile.url
#         fp = open("%s/%s" %(settings.MEDIA_ROOT, file_nm), 'r')
#         # fp.seek(0)
...
#         for line in lines:
#             file_code = file_code + line

#         fp.close()

#         # context = { 'file_code': file_code }
#         request.session['file_code'] = file_code   ### In this way also we can transfer the context of our 'view' to the 'Template' without using 'render'. This is real cool !!        
...
def compile(request):
    ...
    if 'file_code' in request.session:
        del request.session['file_code']    ### This is really important if some session is passed as comtext to the template then it has to be deleted as well.
    ...
    if request.method == 'POST':   
        ...
        if not request.user.is_authenticated:
            ...
            if request.POST.get('uname') and request.POST.get('pwd'):
                ...
                # return render(request, 'compiler/user_base.html')    
                return redirect('home')
        ...
        if request.POST.get('fcode'):

            f_id = int(request.POST.get('fcode'))

            file_id = CodeFileUpload.objects.get(pk=f_id)
            print('\n\nFile_Id (get code) :', file_id.codefile.url, end='\n\n')
            file_nm = file_id.codefile.url
            fp = open("%s/%s" %(settings.MEDIA_ROOT, file_nm), 'r')
            # fp.seek(0)
        ...
        if form.is_valid() or upload.is_valid():                            
            ...
            try:                
                    ...
                    if form.is_valid():
                        ...
                        user_codefile = user_codefile.replace('%20', '_')                         ## **This is becz if 'name' of the 'file' uploaded by the "user" contains "spaces" then at the time of storing it into the LocalFileStorageSystem (F:/django/..../media/Code_Files/filename.ext), the "spaces" are converted into "underscore(s)"(_) ....
                        print('\nuser_codefile (with_underscore) :', user_codefile, end='\n\n')   ##   The "address of the file" is stored in the database(not the file itself), so when we fetch the "file name" from the url(or address) of the file stored in the database then the "underscore(s)" are converted into ("%20") (may be becz of UTF-8 Decoding or Conversion) ... So we need to replace it again with the "underscore(s)" as we are doing in this step.
                    ...
                    ##      For "Python_3" Language.
                        ...
                        if c == b'':  
                            ...
                            # compile_result = c
                            compile_result = c.decode('UTF-8').split('\r\n')
                            print('\n Python3 err =', compile_result, end='\n\n\n' )                                                                
                        ...
                        if c == b'':   ## This will be 'True' if Compilation gives "No Error".
                            ...
                            if c2_error_flag == 1:                            
                                ...
                                if main_not_found_error_count == len(class_files):                                
                                    ...
                                    # compile_result.append('\n')                       
                                    compile_result.append('Program requires for some "input(s)" which is not provided. If it is so, please enter the input(s) in "Inputs(if any)" Green Textbox Field located above.')                       
                ...
                code_output = 'Compilation Error...:-('
                compile_result = [ 'EmptyInputError:', '~ Server Generated Error, (Not Standard) /', error, ' ', 'Program requires for some "input(s)" which is not provided. If it is so, please enter the input(s) in "Inputs(if any)" Green Textbox Field located above.' ]
    ...
    if request.user.is_authenticated:
        ...
        context = { 'form': form, 'iform': iform, 'upload': upload, 'compile': compile_result, 'output': code_output, 'user_code': user_code, 'user_files': user_files }           
    elif request.POST.get('fcode'):
        context = { 'form': form, 'iform': iform, 'upload': upload, 'compile': compile_result, 'output': code_output, 'user_code': user_code, 'user_files': user_files, 'file_code': file_code }           
    else:
